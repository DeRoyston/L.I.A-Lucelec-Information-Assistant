...
def generate_google_tts(text: str, language: str = "English") -> bytes:
    ...
    try:
        ...
        print(f"[TTS DEBUG] gtts import failed: {e}", flush=True)
        return b""
    ...
    for tld in tlds_to_try:
        try:
            print(f"[TTS DEBUG] trying tld={tld}", flush=True)
            ...
            audio = fp.read()
            if audio:
                print(f"[TTS DEBUG] tld={tld} succeeded, {len(audio)} bytes", flush=True)
                return audio
        except Exception as e:
            print(f"[TTS DEBUG] tld={tld} failed: {type(e).__name__}: {e}", flush=True)
            last_error = e
            continue

    print(f"Google TTS Error: all {len(tlds_to_try)} regional endpoints failed ({tlds_to_try}). Last error: {last_error}", flush=True)
    return b""
                ...
                with st.chat_message("assistant", avatar=bot_avatar):
                    ...
                    if st.session_state.accessibility_settings.get("tts"):
                        with st.spinner(t("generating_audio_spinner")):
                            ...
                            print(f"[TTS DEBUG] entered TTS block, engine={engine!r}", flush=True)
                            ...
                            print(f"[TTS DEBUG] audio_data bytes={len(audio_data) if audio_data else 0}", flush=True)
