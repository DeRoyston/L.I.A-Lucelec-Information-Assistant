...
def streamlit_app():
    import streamlit as st                           # imported here so --demo works without it

    st.set_page_config(page_title=f"{BOT_NAME} · LUCELEC",   # the browser tab title
                       page_icon="⚡", layout="wide")        # the tab icon, and use the full width
    st.title(f"⚡ {BOT_NAME} — LUCELEC Appliance Cost Assistant")   # the big heading
    st.caption(f"Pod: {POD} · Client: {CLIENT} · "            # the small grey line underneath
               "Answers come only from source_documents/")

    @st.cache_resource                               # "cache" = do this once, then reuse the result
    def get_index():                                 # reading and scoring every document is slow,
        return build_index(load_chunks())            # so we do it once instead of on every click

    index = get_index()                              # get the index (fast after the first time)

    # ---------- SIDEBAR: settings, keys, documents ----------
    with st.sidebar:                                 # everything indented here goes in the sidebar
        st.subheader("AI model")                     # a small heading

        names  = [p["name"] for p in provider_status()]   # ["openai", "gemini", "groq", ...]
        labels = {p["name"]: f"{p['label']} {'✅' if p['ready'] else '⚪'}"   # tick if ready
                  for p in provider_status()}        # a lookup of name -> label with tick

        current = os.getenv("LLM_PROVIDER", "")      # the currently pinned provider, if any
        choice = st.selectbox("Provider", names,     # the dropdown menu
                              format_func=lambda n: labels[n],   # show the pretty label, not the id
                              index=names.index(current) if current in names else 0)
        os.environ["LLM_PROVIDER"] = choice          # remember the choice for this session
        cfg = PROVIDERS[choice]                      # the settings for whichever was chosen

        model_in = st.text_input("Model", value=provider_model(choice),   # editable model name
                                 help="Model IDs change. Check the provider's docs on a 404.")
        os.environ[cfg["model_env"]] = model_in.strip()   # apply the model name immediately

        try:                                         # is the openai library installed?
            import openai                            # noqa: F401 — we only want to know
            sdk_ok = True                            # yes: the reliable route is available
        except ImportError:                          # no
            sdk_ok = False                           # we will fall back to built-in urllib
        if not sdk_ok:                               # warn, because this causes odd 403s
            st.warning("The `openai` package is missing, so requests go out through "
                       "Python's built-in fetcher. Some providers sit behind "
                       "Cloudflare and refuse that. Run `pip install openai`.")

        if st.button("Test connection"):             # a button; True on the click that pressed it
            r = test_provider(choice)                # send a tiny test message
            (st.success if r["ok"] else st.error)(f"{r['model']} → {r['reply']}")   # green or red

        st.caption("Fallback order: " + " → ".join(  # show the order providers will be tried in
            PROVIDERS[n]["label"] for n in active_chain()) + " → offline")

        # ---------- API KEYS: stored on the server, never in the code ----------
        with st.expander("🔑 API keys"):             # a collapsible section, closed by default
            st.caption("Keys are saved to `.streamlit/secrets.toml` on the server. "
                       "They are never shown in full and never go in your code.")

            for row in key_report():                 # one line per key we know about
                if row["set"]:                       # is this key set?
            ...
            # Pick which key to add or change.
            target = st.selectbox("Add or replace a key", KEY_NAMES,
                                  index=KEY_NAMES.index(cfg["key_env"])   # default to the chosen provider's
                                  if cfg["key_env"] in KEY_NAMES else 0)

            new_key = st.text_input("Paste the key", type="password",     # type=password hides it
                                    help=f"Get one at {cfg['get_key']}")

            col_save, col_del = st.columns(2)        # put two buttons side by side
            with col_save:                           # the left column
                if st.button("Save to server"):      # the Save button
                    if new_key.strip():              # only if something was actually typed
                        path = save_key(target, new_key)          # write it to the back end
                        st.success(f"Saved to {path}")            # confirm, without echoing the key
                        st.rerun()                                # refresh so the list updates
                    else:                            # the box was empty
                        st.warning("Nothing to save — the box is empty.")
            with col_del:                            # the right column
                if st.button("Delete"):              # the Delete button
                    delete_key(target)               # remove it from the file and from memory
                    st.info(f"{target} removed.")    # confirm
                    st.rerun()                       # refresh the display

            st.caption("⚠️ Anyone who can open this Deepnote project can read that file. "
                       "Use the camp-managed key, never a personal one, and never share "
                       "a duplicated project that still has a key inside it.")

        st.divider()                                 # separates settings from documents
        st.subheader("Knowledge base")               # heading for the documents section
        st.metric("Chunks indexed", len(index["chunks"]))   # a big number showing chunk count

        st.write("Documents:")                       # a label
        for s in sorted({c["source"] for c in index["chunks"]}):   # {} removes duplicate filenames
            st.write(f"· {s}")                       # list each document

        if st.button("Reload documents"):            # press after editing source_documents/
            st.cache_resource.clear()                # throw away the cached index
            st.rerun()                               # start the page again, rebuilding it

        st.divider()                                 # separator
        st.subheader("The user in front of you")     # heading for the A.R.T. inputs
        st.caption("These three answers ARE the A.R.T. axes. Change them and "
                   "watch the bot's voice and its escalation behaviour change.")

        sim_verified = st.checkbox("Authority — identity verified", value=True,
                                   help="Untick this and every question escalates. That is correct.")

        sim_mood = st.selectbox("Register — how do they feel?",           # Axis 2
                                ["(read it from the message)"] + list(TONES.keys()),
                                help="Pick a register to force it, or let the bot infer it.")

        sim_segment = st.selectbox("Territory — which customer category?",   # Axis 3
                                   ["(unknown)"] + list(LOCATION_CONTEXT.keys()),
                                   help="Choose (unknown) to see the Territory escalation.")

        user = {                                     # the dictionary the A.R.T. checks read
            "id_verified": sim_verified,             # Axis 1 input
            "mood":    None if sim_mood.startswith("(") else sim_mood,      # Axis 2 input
            "segment": None if sim_segment.startswith("(") else sim_segment,# Axis 3 input
        }

        st.divider()                                 # separator
        st.subheader("The Red Line")                 # heading for the safety rules
        for r in MUST_NEVER_DO:                      # go through each rule
            st.write(f"· {r}")                       # display it, so the client can see it

    # ---------- MAIN AREA: four tabs ----------
    (tab_chat, tab_calc, tab_blueprint,              # six tabs across the top
     tab_web, tab_sources, tab_eval) = st.tabs(
        ["Chat", "Cost calculator", "Blueprint", "Web harvest", "Sources", "Eval"])

    # ---------- TAB 1: the chat ----------
    with tab_chat:                                   # everything indented here is in the Chat tab
        if "messages" not in st.session_state:       # session_state survives clicks; normal variables do not
            st.session_state.messages = []           # first visit: start with an empty history

        for m in st.session_state.messages:          # redraw the whole conversation so far
            with st.chat_message(m["role"]):         # a speech bubble, "user" or "assistant"
                st.markdown(m["content"])            # the message text
                if m.get("hits"):                    # if this message had sources
                    with st.expander("Sources used"):        # a collapsible section
                        for i, h in enumerate(m["hits"], 1): # each source, numbered from 1
                            st.markdown(f"**[{i}] {h['source']}** · score {h['score']}")
                            st.caption(h["text"])    # the actual chunk text, in small grey type

        if q := st.chat_input("Ask about appliance costs, tariffs or saving energy…"):
            # ":=" stores the typed text in q AND checks it is not empty, in one step.
            st.session_state.messages.append({"role": "user", "content": q})   # save the question
            with st.chat_message("user"):            # draw the person's bubble
                st.markdown(q)                       # showing what they asked

            with st.chat_message("assistant"):       # draw the bot's bubble
                with st.spinner("Retrieving…"):      # show a spinner while it thinks
                    out = answer(q, index, user)     # THE ACTUAL BOT CALL — A.R.T. first, then the AI
                st.markdown(out["reply"])            # show the answer
                st.caption(f"lane: {out.get('intent', '-')} · "               # social or domain
                           f"register: {out.get('register', '-')} · "        # which outfit it wore
                           f"answered by: {out['provider']} · {out['model']}")  # and who answered
                if out["hits"]:                      # if there were sources
                    with st.expander("Sources used"):        # show them, collapsed
                        for i, h in enumerate(out["hits"], 1):
                            st.markdown(f"**[{i}] {h['source']}** · score {h['score']}")
                            st.caption(h["text"])

            st.session_state.messages.append(        # save the answer so it survives the next click
                {"role": "assistant", "content": out["reply"], "hits": out["hits"]})

    # ---------- TAB 2: the calculator ----------
    with tab_calc:                                   # the Cost calculator tab
        st.warning("The rate below is a placeholder. Confirm the real LUCELEC rate "
                   "with the client before you show this to anyone.")

        rate = st.number_input("Rate (EC$ per kWh)", value=float(DEFAULT_RATE_PER_KWH),
                               min_value=0.0, step=0.05)   # step = how much the +/- buttons move

        c1, c2 = st.columns(2)                       # split the page into two columns
        with c1:                                     # the left column: appliance A
            st.subheader("Appliance A")              # heading
            na = st.text_input("Name A", "Old fridge")            # a text box with a default
            wa = st.number_input("Watts A", value=350.0, min_value=0.0)          # power draw
            ha = st.number_input("Hours per day A", value=8.0, min_value=0.0, max_value=24.0)
            pa = st.number_input("Purchase price A (EC$)", value=1500.0, min_value=0.0)
        with c2:                                     # the right column: appliance B
            st.subheader("Appliance B")
            nb = st.text_input("Name B", "Inverter fridge")
            wb = st.number_input("Watts B", value=150.0, min_value=0.0)
            hb = st.number_input("Hours per day B", value=8.0, min_value=0.0, max_value=24.0)
            pb = st.number_input("Purchase price B (EC$)", value=2600.0, min_value=0.0)

        if st.button("Compare"):                     # only run the maths when asked
            result = compare_appliances(             # call the calculator from Section 8
                {"name": na, "watts": wa, "hours_per_day": ha, "price": pa},   # appliance A
                {"name": nb, "watts": wb, "hours_per_day": hb, "price": pb},   # appliance B
                rate)                                # the rate typed in above

            k1, k2, k3 = st.columns(3)               # three columns for three headline numbers
            k1.metric(f"{na} — per year", f"EC${result[na]['cost_year']:,.2f}")   # :,.2f = 1,234.56
            k2.metric(f"{nb} — per year", f"EC${result[nb]['cost_year']:,.2f}")
            k3.metric("Yearly saving", f"EC${result['yearly_saving']:,.2f}")

            if result["payback_years"]:              # only if a payback time makes sense
                st.success(f"{result['cheaper_to_run']} pays back the price difference in about "
                           f"{result['payback_years']} years.")
            st.json(result)                          # show every number, for checking the maths

    # ---------- TAB 3: the Week 1 Blueprint ----------
    with tab_blueprint:                              # the Blueprint tab
        st.caption("Everything your Pod decided in Week 1, read straight out of "
                   "Section 2 of the code. If a line still says TODO_, it is not finished.")

        b1, b2 = st.columns(2)                       # two columns side by side
        with b1:                                     # left column: who and how they feel
            st.subheader("Primary persona")          # Day 2, Driving Question 1
            for k, v in PERSONA.items():             # go through every field
                st.write(f"**{k}** — {v}")           # show it as a labelled line

        with b2:                                     # right column: the empathy map
            st.subheader("Empathy map")              # Day 2, Driving Question 2
            for k, v in EMPATHY_MAP.items():         # says / thinks / does / feels
                st.write(f"**{k}** — {v}")

        st.divider()                                 # separator

        st.subheader("The Wardrobe — code-switch demo")   # the Day 2 Micro-Pitch, live
        st.caption("Same facts. Different outfit. This is the demo to show the client.")

        sample = st.text_input("A line to say in every register",       # one sentence...
                               "Your fridge costs about EC$438 a year to run.")
        for register_name in TONES:                  # ...said once per register
            st.write(f"`{register_name}` → {dress(register_name, sample)}")

        st.divider()                                 # separator

        c1, c2 = st.columns(2)                       # two more columns
        with c1:                                     # left: what the bot remembers
            st.subheader("Memory & privacy")         # Day 2 Responsible AI beat
            st.write("**Held during the conversation**")
            for item in MEMORY_POLICY["remember_during_the_conversation"]:
                st.write(f"· {item}")                # one line each
            st.write("**Never stored**")
            for item in MEMORY_POLICY["never_store"]:
                st.write(f"· {item}")
            st.caption(MEMORY_POLICY["forgotten_when"])   # the promise, in small type

        with c2:                                     # right: what is still unconfirmed
            st.subheader("Confirmation register")    # Day 3 and Day 4 stickers
            outstanding = open_confirmations()       # facts with no client sign-off
            if outstanding:                          # anything still open?
                st.error(f"{len(outstanding)} facts still unconfirmed")   # red, because it matters
                st.dataframe(outstanding, use_container_width=True)       # show them as a table
            else:                                    # nothing outstanding
                st.success("Every fact signed off by the client.")        # green, and rare

        st.divider()                                 # separator

        st.subheader("Day 3 interview")              # the two questions you prepared
        for kind, qa in INTERVIEW_QUESTIONS.items(): # "fact" and "tone"
            st.write(f"**{kind.upper()}** — {qa['asked']}")   # the question you asked
            st.caption(f"Client said: {qa['answer']}")        # what came back

        st.divider()                                 # separator

        st.subheader("Location & Context rulebook")  # Day 4, the Territory axis
        st.dataframe([{"segment": k, **v} for k, v in LOCATION_CONTEXT.items()],
                     use_container_width=True)       # ** merges the rules into the row

        st.subheader("Jargon → plain language")      # Day 4, Dignity in Words
        st.dataframe([{"client word": k, "what we say": v}   # one row per translation
                      for k, v in JARGON_TO_PLAIN.items()],
                     use_container_width=True)

    # ---------- TAB 4: harvesting from the client's website ----------
    with tab_web:                                    # the Web harvest tab
        st.subheader("Harvest a page from the client's website")
        st.info("Downloading a page is not the same as confirming a fact. Pages land "
                "in the waiting room below. Nothing reaches the bot until a human "
                "approves it with their initials.")

        st.caption(f"Allowed hosts: {', '.join(sorted(allowed_hosts()))} · "
                   f"robots.txt is checked every time · "
                   f"we wait whatever the site asks for, minimum "
                   f"{FETCH_DELAY_SECONDS}s (LUCELEC asks for 10s)")

        url = st.text_input("Page address",          # the box you paste a URL into
                            "https://www.lucelec.com/content/customer-service")

        if st.button("Fetch page"):                  # only fetch when asked
            # This pause can be long. LUCELEC's robots.txt asks for 10 seconds
            # between pages, and we obey the site's number rather than our own.
            with st.spinner("Checking robots.txt, waiting out the site's crawl "
                            "delay, then downloading…"):
                res = harvest(url)                   # all the safety checks live in here
            if res["ok"]:                            # it worked
                st.success(f"Saved {res['words']} words to {res['path']} — awaiting approval")
                st.text(res["preview"])              # show the first part so you can eyeball it
            else:                                    # refused or failed
                st.error(res["error"])               # say exactly why
                if "robots.txt" in res["error"]:     # if robots.txt was the blocker
                    with st.expander("What did robots.txt actually say?"):
                        st.json(check_robots(url))   # show the full diagnosis

        st.divider()                                 # separator

        st.subheader("Waiting room")                 # everything fetched but not approved
        pending = list_pending()                     # read the pending_review/ folder

        if not pending:                              # nothing waiting
            st.caption("Empty. Nothing is waiting for approval.")
        for item in pending:                         # one block per pending page
            with st.expander(f"{item['file']} — {item['words']} words — {item['url']}"):
                st.text(item["preview"])             # a preview of the text
                st.caption("Read it properly before you approve it. Your initials go in the file.")

                col_i, col_a, col_r = st.columns([2, 1, 1])   # initials box + two buttons
                with col_i:                          # the initials box
                    who = st.text_input("Your initials", key=f"who_{item['file']}",
                                        max_chars=4, label_visibility="collapsed",
                                        placeholder="Your initials")
                with col_a:                          # the approve button
                    if st.button("Approve", key=f"ok_{item['file']}"):
                        r = approve_harvest(item["file"], who)     # the only door in
                        if r["ok"]:                  # approved
                            st.success(f"Approved by {r['by']} → {r['path']}")
                            st.cache_resource.clear()    # the knowledge base changed
                            st.rerun()               # rebuild the page
                        else:                        # refused, usually no initials
                            st.error(r["error"])
                with col_r:                          # the reject button
                    if st.button("Reject", key=f"no_{item['file']}"):
                        reject_harvest(item["file"])  # bin it
                        st.rerun()                   # refresh the list

    # ---------- TAB 5: testing retrieval ----------
    with tab_sources:                                # the Sources tab
        st.write("Drop the client's own .md or .txt files into `source_documents/`, "
                 "then press Reload documents in the sidebar.")

        probe = st.text_input("Test a retrieval query",          # a box to test searching alone
                              "how much does an air conditioner cost to run")

        for i, h in enumerate(retrieve_chunks(probe, index), 1): # run retrieval, no AI involved
            st.markdown(f"**[{i}] {h['source']}** · score {h['score']}")   # which chunk, what score
            st.caption(h["text"])                    # the chunk text, so you can judge it yourself

    # ---------- TAB 6: the eval ----------
    with tab_eval:                                   # the Eval tab
        st.write("Every change you make, run this again. If a row flips from "
                 "pass to fail, you broke something.")

        if st.button("Run eval set"):                # only run when asked; it costs API credit
            rows = run_eval(index, user)             # ask every test question as this user
            st.dataframe(rows, use_container_width=True)         # show the results as a table
            passed = sum(1 for r in rows if r["passed"])         # count how many passed
            st.metric("Passed", f"{passed}/{len(rows)}")         # display the score
